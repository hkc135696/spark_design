  ...
  <div class="region-rank">
    ...
    <div class="table-wrapper">
      <table class="data-table">
        <thead>
          <tr>
            <th>排行</th><th>区域</th><th>日期</th><th>小时</th>
            <th>平均拥堵指数</th><th>总流量</th><th>平均车速</th><th>活跃道路</th><th>热度评分</th><th>更新时间</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="row in data" :key="row.district + row.update_time">
            <td class="rank">{{ row.rank_position }}</td>
            <td>{{ row.district }}</td>
            <td>{{ row.rank_date }}</td>
            <td class="num">{{ row.rank_hour }}:00</td>
            <td class="num" :class="congestionClass(row.avg_congestion)">{{ row.avg_congestion }}</td>
...
import { getRegionRankList } from '../../utils/api.js'
...
async function loadData() {
  try { data.value = await getRegionRankList() } catch (e) { console.error(e) }
}
...
<style scoped>
.region-rank { background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 20px; }
